...
def find_package_mention_v2(pack, words):
    if pack in words:
        return 1
    return 0
...
def find_all_packages_mentions(packages, transcripts_path, episodes):
    ...
    for pack in packages:
        ...
        for i in os.listdir(transcripts_path):
            ...
            mention = find_package_mention_v2(pack, transcript)
...
def find_all_packages_mentions_v2(packages, transcripts_path, episodes):
    num = 0
    all_transcripts = os.listdir(transcripts_path)
    for i in all_transcripts:
        display_progress(num, len(all_transcripts))
        num += 1
        path = os.path.join(transcripts_path, i)
        transcript = read_file(path)
        words = set(transcript.lower().split())
        for pack in packages:
            mention = find_package_mention_v2(pack, words)
            packages[pack] += mention
            if mention:
                episode_number = extract_episode_number_from_title(i)
                episodes[episode_number].package_mentioned.append(pack)
...
def main():
    ...
    find_all_packages_mentions_v2(packages, transcripts_path, episodes)
